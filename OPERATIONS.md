...
## 部署
- **本次發布授權（2026-09-08）**：使用者已同意將本輪完成的 SEO 改版推送並上線，明確指示先略過 Vercel 商用資格確認。此項依使用者指示延後，不代表資格已核實；不變更方案或帳務。正式 GA4 未取得 Measurement ID，維持關閉。
...
## SEO
- [90 天 SEO 執行計畫](docs/SEO_PLAN_2026-09-08.md)：含公開網站檢查、關鍵字與頁面配置、車款獨立頁規格及成效量測。首頁定位、六款車頁、北投到店頁、四篇指南及事件程式已在本機實作；尚未部署或啟用正式 GA4。量測基準與發布步驟見 [SEO 量測作業](docs/SEO_MEASUREMENT.md)；不保證排名。
...
4. **成效資料**：先定義 LINE 點擊、電話點擊、表單確認收件等事件，再配置統計。只傳事件及車款，不傳姓名、電話或表單內容；點擊不代表已成交。事件程式已實作，正式 GA4 資料串流及 Measurement ID 尚未設定；見下方 SEO 改版紀錄。
5. **車款獨立頁與內容（本機已完成）**：六款車、到店頁及四篇指南已有獨立網址、metadata 與 sitemap。下一步為顧問核對內容、授權發布及觀察逐頁收錄與有效諮詢；不能保證排名。[Google SEO 入門](https://developers.google.com/search/docs/fundamentals/seo-starter-guide)

...
**2026-09-20 後續更正**：業主確認下列唯一 1 次 `generate_lead` 為本人測試。GA4 原始事件仍為 1，排除測試後，該期已確認真實客戶表單諮詢為 0；LINE／電話點擊不算有效案件。另，本報告的內鏈與 JavaScript 改善已於提交 `5c6e794` 正式發布，Vercel Ready 與正式站驗收見 [維運紀錄](../OPERATIONS.md)。本文其餘本機驗證與發布前描述保留 9/19 時點；後續新增的 Jimny／SWIFT 購車意圖內容與驗證另見維運紀錄，不與本報告的前批部署混算。
...
| 電話點擊 `phone_click` | 1 |
| 表單收件 `generate_lead`（原始事件） | 1 |
| 已知測試表單（9/20 業主確認） | 1 |
| 排除測試後已確認真實客戶表單諮詢 | 0 |

`generate_lead` 已在報表中單獨選取確認，不是從所有事件推估。9/19 尚未核實案件；9/20 業主確認為本人測試，因此不能當成客戶購車需求或 SEO 轉換成果。本次依業主回覆更正解讀，未讀私人案件或外部通知紀錄、未刪除或變更 GA4 歷史資料。LINE、電話點擊也不能和收件相加當作三位客戶。
...
以下保留 9/19 排程。9/20 更新：第 1 項已發布並驗收，第 3 項唯一收件已確認為測試；接續優先改善 Jimny／SWIFT 高購車意圖內容，並以實際發布後完整 28 天對發布前 28 天追蹤，詳見 [量測作業](SEO_MEASUREMENT.md#購車意圖頁追蹤2026-09-20-起)。
